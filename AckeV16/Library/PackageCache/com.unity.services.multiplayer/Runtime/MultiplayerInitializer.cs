...
namespace Unity.Services.Multiplayer
{
    partial class MultiplayerInitializer : IInitializablePackageV2
    {
        ...
        void InitializeServices(CoreRegistry registry, bool globalRegistry)
        {
            ...
            var daService = InitializeDaService(accessToken, projectConfiguration, lobbyService, relayService, qosResults);
            ...
            var m_SessionQuerier = new SessionQuerier(actionScheduler, lobbyService);
            var m_SessionManager = new SessionManager(actionScheduler, moduleRegistry, lobbyBuilder, playerId, accessTokenObserver);
            var m_MatchmakerManager = new MatchmakerManager(m_SessionManager, actionScheduler, qosService, matchmakerService, playerId, accessToken, accessTokenObserver);
            var multiplayerService = new WrappedMultiplayerService(m_SessionQuerier, m_SessionManager, m_MatchmakerManager, moduleRegistry);
